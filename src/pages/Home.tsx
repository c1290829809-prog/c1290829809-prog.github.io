...
export function HomePage(){
 ...
 return <main className="min-h-[100dvh] bg-paper pb-28">
  <header className="px-5 pt-8 lg:px-10 lg:pt-12"><h1 className="text-5xl font-black tracking-[-.08em] lg:text-6xl">循迹 <span className="text-2xl text-[#f4b321]">✦ ✦</span></h1><p className="mt-2 text-sm text-stone-500">沿着喜欢的人，重演认识一座城市</p><div className="mt-5 grid grid-cols-[1fr_56px] gap-3 lg:max-w-3xl"><SearchBox onClick={()=>nav('/search')}/><button className="flex items-center justify-center rounded-[20px] bg-white shadow-soft active:scale-95"><Bell size={21}/></button></div></header>
...
function Section({title,link,children}:{title:string;link?:string;children:React.ReactNode}){return <section className="mt-7 px-5 lg:px-10"><div className="mb-3 flex items-center justify-between"><h2 className="text-lg font-black">{title}</h2>{link&&<Link to="/search" className="flex items-center text-xs text-stone-400">{link}<ChevronRight size={15}/></Link>}</div>{children}</section>}
