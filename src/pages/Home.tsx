...
export function HomePage(){
 ...
 return <main className="min-h-[100dvh] bg-paper pb-28">
  <header className="px-5 pt-8 lg:px-10 lg:pt-12"><h1 className="xj-display text-5xl font-black lg:text-6xl">循迹 <span className="text-2xl text-[#f4b321]">✦ ✦</span></h1><p className="mt-3 text-sm leading-6 text-stone-500">沿着喜欢的人，重演认识一座城市</p><div className="mt-5 grid grid-cols-[1fr_56px] gap-3 lg:max-w-3xl"><SearchBox onClick={()=>nav('/search')}/><button className="xj-search xj-press flex items-center justify-center rounded-[22px]"><Bell size={21} strokeWidth={1.75}/></button></div></header>
  ...
  <Section title="同款地点" link="全部地点"><div className="grid grid-cols-2 gap-3 lg:grid-cols-4 lg:gap-4">{featured.map(({place,relation,idol})=><article key={place.id} className="xj-surface xj-press xj-lift overflow-hidden"><Link to={`/place/${place.id}`}><img src={place.images[0]} className="h-28 w-full object-cover"/><div className="p-3"><h3 className="truncate font-bold">{place.name}</h3><p className="mt-1 truncate text-[11px] text-stone-500">{place.city||'深圳'} · {place.address.slice(0,5)}</p><p className="mt-2 text-xs">{idol} <span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] text-orange-700">{relation.relationType==='filming'?'取景地':'公开分享'}</span></p><p className="mt-2 truncate text-[10px] text-stone-400">#城市漫游 #公开可查</p><div className="mt-2"><CredibilityBadge level={relation.credibility}/></div></div></Link><div className="grid grid-cols-2 border-t border-stone-100 text-[11px] font-semibold"><button onClick={()=>toggle(place.id)} className="flex min-h-11 items-center justify-center gap-1 border-r active:bg-stone-50"><Heart size={14} fill={favs.includes(place.id)?'currentColor':'none'}/>收藏</button><button onClick={()=>add(place.id)} className="flex min-h-11 items-center justify-center gap-1 active:bg-stone-50"><Plus size={14}/>路线</button></div></article>)}</div></Section>
  <Section title="🔥 热门城市">{basic.cities.length?<div className="grid grid-cols-2 gap-3">{[...basic.cities].sort((a,b)=>b.placeCount-a.placeCount).slice(0,4).map(city=>{const cityImage=city.cover||published.find(place=>place.city===city.name)?.images[0]||cityFallbackImages[city.name]||places[0].images[0];return <Link to={`/city/${encodeURIComponent(city.id)}`} key={city.id} className="group relative min-h-28 overflow-hidden rounded-[20px] bg-stone-200 shadow-soft ring-1 ring-white/70 transition hover:-translate-y-0.5 active:scale-95"><img src={cityImage} alt={`${city.name}城市风景`} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-black/20 via-transparent to-black/5"/><div className="absolute inset-y-2 left-2 z-10 flex w-[58%] flex-col justify-center rounded-[16px] border border-white/55 bg-white/[.72] px-3 shadow-[inset_0_1px_0_rgba(255,255,255,.75),0_8px_24px_rgba(35,30,25,.08)] backdrop-blur-xl"><b className="text-stone-900">{city.name}</b><p className="mt-2 text-[11px] text-stone-600">{city.placeCount} 个地点</p><p className="text-[11px] text-stone-600">{city.routeCount} 条路线</p></div></Link>})}</div>:<div className="rounded-[20px] bg-white px-5 py-8 text-center text-sm text-stone-400 shadow-soft">后台录入城市后会显示在这里</div>}</Section>
...
function Section({title,link,children}:{title:string;link?:string;children:React.ReactNode}){return <section className="mt-[clamp(2.125rem,6vw,3.5rem)] px-5 lg:px-10"><div className="mb-4 flex items-center justify-between"><h2 className="xj-title text-lg font-bold">{title}</h2>{link&&<Link to="/search" className="xj-press flex min-h-11 items-center text-xs text-stone-400">{link}<ChevronRight size={15}/></Link>}</div>{children}</section>}
