...
import {BottomNav,CredibilityBadge,haptic,SearchBox} from '../components/ui'
...
const cityFallbackImages:Record<string,string>={
 深圳:'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee?auto=format&fit=crop&w=480&q=82',
 香港:'https://images.unsplash.com/photo-1536599018102-9f803c140fc1?auto=format&fit=crop&w=480&q=82',
 上海:'https://images.unsplash.com/photo-1548919973-5cef591cdbc9?auto=format&fit=crop&w=480&q=82',
 重庆:'https://images.unsplash.com/photo-1573455494060-c5595004fb6c?auto=format&fit=crop&w=480&q=82',
 北京:'https://images.unsplash.com/photo-1508804185872-d7badad00f7d?auto=format&fit=crop&w=480&q=82'
}
export function HomePage(){
 ...
 return <main className="xj-page min-h-[100dvh] bg-paper pb-28 lg:pb-12">
  <header className="px-5 pt-10 md:px-8 lg:px-10 lg:pt-14"><p className="text-xs font-semibold tracking-wide text-[var(--xj-accent)]">循迹城市漫游</p><h1 className="xj-display mt-2 text-5xl font-bold lg:text-6xl">循迹</h1><p className="mt-3 text-sm leading-6 text-[var(--xj-text-2)] lg:text-base">沿着喜欢的人，重演认识一座城市</p><div className="mt-6 grid grid-cols-[1fr_56px] gap-3 lg:max-w-xl"><SearchBox onClick={()=>nav('/search')}/><button onClick={()=>haptic()} className="xj-search xj-press flex items-center justify-center rounded-[20px]"><Bell size={21} strokeWidth={1.75}/></button></div></header>
  ...
  <Section title="同款地点" link="全部地点"><div className="grid grid-cols-2 gap-3 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3">{featured.map(({place,relation,idol})=><article key={place.id} className="xj-surface xj-press xj-lift overflow-hidden lg:grid lg:grid-cols-[180px_1fr] lg:items-stretch"><Link to={`/place/${place.id}`} className="contents"><img src={place.images[0]} className="h-28 w-full object-cover lg:h-full lg:min-h-40"/><div className="p-3 lg:p-5"><h3 className="truncate font-bold lg:text-lg">{place.name}</h3><p className="mt-1 truncate text-[11px] text-stone-500 lg:text-xs">{place.city||'深圳'} · {place.address.slice(0,12)}</p><p className="mt-2 text-xs">{idol} <span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] text-orange-700">{relation.relationType==='filming'?'取景地':'公开分享'}</span></p><p className="mt-2 truncate text-[10px] text-stone-400">#城市漫游 #公开可查</p><div className="mt-2"><CredibilityBadge level={relation.credibility}/></div></div></Link><div className="col-span-2 grid grid-cols-2 border-t border-stone-100 text-[11px] font-semibold"><button onClick={()=>toggle(place.id)} className="flex min-h-11 items-center justify-center gap-1 border-r active:bg-stone-50"><Heart size={14} fill={favs.includes(place.id)?'currentColor':'none'}/>收藏</button><button onClick={()=>add(place.id)} className="flex min-h-11 items-center justify-center gap-1 active:bg-stone-50"><Plus size={14}/>路线</button></div></article>)}</div></Section>
  <Section title="热门城市">{basic.cities.length?<div className="grid grid-cols-2 gap-3 lg:grid-cols-4">{[...basic.cities].sort((a,b)=>b.placeCount-a.placeCount).slice(0,4).map(city=>{const cityImage=city.cover||published.find(place=>place.city===city.name)?.images[0]||cityFallbackImages[city.name]||places[0].images[0];return <Link to={`/city/${encodeURIComponent(city.id)}`} key={city.id} className="group relative min-h-28 overflow-hidden rounded-[20px] bg-stone-200 shadow-soft transition active:scale-95 lg:min-h-40"><img src={cityImage} alt={`${city.name}城市风景`} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-black/35 via-black/5 to-transparent"/><div className="absolute inset-x-3 bottom-3 z-10"><b className="text-sm text-white lg:text-lg">{city.name}</b><p className="mt-1 text-[11px] text-white/80">{city.placeCount} 个地点 · {city.routeCount} 条路线</p></div></Link>})}</div>:<div className="rounded-[20px] bg-white px-5 py-8 text-center text-sm text-stone-400 shadow-soft">后台录入城市后会显示在这里</div>}</Section>
...
function RecommendedPlaceCarousel({idolName,places}:{idolName:string;places:Place[]}){
 ...
 return <><div className="hidden lg:grid lg:grid-cols-2 lg:gap-5">{places.slice(0,2).map((place,index)=><Link to={`/place/${place.id}`} key={place.id} className="relative h-64 overflow-hidden rounded-[30px] bg-stone-200 shadow-soft transition active:scale-[.99]"><>{place.images[0]?<img src={place.images[0]} alt={place.name} className="h-full w-full object-cover"/>:<div className="flex h-full items-center justify-center text-5xl font-bold text-[var(--xj-accent)]">{place.name.slice(0,2)}</div>}</><span className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/15 to-transparent"/><div className="absolute inset-x-0 bottom-0 p-6 text-white"><p className="text-xs font-semibold text-white/75">{idolName} 的关联地点</p><h2 className="mt-1 text-2xl font-bold">{place.name}</h2><p className="mt-2 text-sm text-white/80">{place.city||'深圳'} · {place.address}</p><p className="mt-3 text-xs text-white/80">关联地点 {index+1} / {places.length}</p></div></Link>)}</div><article className="relative h-52 overflow-hidden rounded-[26px] bg-gradient-to-br from-amber-100 to-orange-200 shadow-soft lg:hidden">
...
function Section({title,link,children}:{title:string;link?:string;children:React.ReactNode}){return <section className="mt-[clamp(2.125rem,6vw,3.5rem)] px-5 md:px-8 lg:px-10"><div className="mb-4 flex items-center justify-between"><h2 className="xj-title text-lg font-bold lg:text-xl">{title}</h2>{link&&<Link to="/search" className="xj-press flex min-h-11 items-center text-xs text-stone-400">{link}<ChevronRight size={15}/></Link>}</div>{children}</section>}
