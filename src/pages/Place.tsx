...
<div className="xj-glass fixed bottom-[88px] left-1/2 z-30 grid w-[calc(100%-1rem)] -translate-x-1/2 grid-cols-[1.1fr_1fr] gap-3 rounded-[22px] border border-white/60 p-2.5 md:w-[414px] md:max-w-[calc(100%-3rem)]"><button onClick={()=>add(place.id)} className="xj-press flex min-h-14 items-center justify-center gap-2 rounded-[16px] bg-orange-500 font-bold text-white"><Route/>加入路线</button><button className="xj-press min-h-14 rounded-[16px] border border-stone-300/80 bg-white/80 font-bold">补充证据 / 纠错</button></div>{shareOpen&&<ShareSheet placeName={place.name} close={()=>setShareOpen(false)} systemShare={systemShare} wechat={()=>copyLink('链接已复制，请打开微信粘贴发送')} copy={()=>copyLink()}/>} {notice&&<div role="status" className="fixed left-1/2 top-20 z-[80] w-max max-w-[85%] -translate-x-1/2 rounded-full bg-stone-900 px-5 py-3 text-center text-sm font-medium text-white shadow-xl">{notice}</div>}<BottomNav/></main>
}
function PlaceGallery({images,name,children}:{images:string[];name:string;children:React.ReactNode}){
 ...
 return <div className="relative h-[330px] overflow-hidden rounded-[28px] bg-stone-200" onTouchStart={event=>{touchStart.current=event.touches[0]?.clientX??null}} onTouchEnd={event=>{const start=touchStart.current,end=event.changedTouches[0]?.clientX;touchStart.current=null;if(start!==null&&end!==undefined&&Math.abs(end-start)>36)move(end<start?1:-1)}}>
...
return <div className="fixed inset-0 z-[70] bg-black/30 backdrop-blur-[3px]" onClick={close}><section role="dialog" aria-modal="true" aria-label={`分享 ${placeName}`} onClick={event=>event.stopPropagation()} className="xj-glass absolute bottom-0 left-1/2 w-full max-w-xl -translate-x-1/2 rounded-t-[28px] border border-white/60 p-5 pb-8"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-lg font-black">分享地点</h2><p className="mt-1 max-w-64 truncate text-xs text-stone-400">{placeName}</p></div><button type="button" onClick={close} aria-label="关闭分享" className="xj-press flex h-11 w-11 items-center justify-center rounded-full bg-white/70"><X size={19}/></button></div><div className="grid grid-cols-3 gap-3"><ShareOption icon={<MessageCircle/>} label="微信" color="bg-[#22a559]" click={wechat}/><ShareOption icon={<Share2/>} label="更多分享" color="bg-orange-500" click={systemShare}/><ShareOption icon={<Copy/>} label="复制链接" color="bg-stone-800" click={copy}/></div><p className="mt-5 rounded-2xl bg-white/55 p-3 text-center text-[11px] leading-5 text-stone-500">手机端点击“更多分享”可调用系统分享菜单；桌面端分享微信会复制链接。</p></section></div>
}
