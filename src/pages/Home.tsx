...
export function HomePage(){
 ...
 return <main className="xj-page min-h-[100dvh] bg-paper pb-28">
  <header className="px-5 pt-10"><p className="text-xs font-semibold tracking-wide text-[var(--xj-accent)]">循迹城市漫游</p><h1 className="xj-display mt-2 text-5xl font-bold">循迹</h1><p className="mt-3 text-sm leading-6 text-[var(--xj-text-2)]">沿着喜欢的人，重演认识一座城市</p><div className="mt-6 grid grid-cols-[1fr_56px] gap-3"><SearchBox onClick={()=>nav('/search')}/><button onClick={()=>haptic()} className="xj-search xj-press flex items-center justify-center rounded-[20px]"><Bell size={21} strokeWidth={1.75}/></button></div></header>
  ...
  <Section title="热门城市">{basic.cities.length?<div className="grid grid-cols-2 gap-3">{[...basic.cities].sort((a,b)=>b.placeCount-a.placeCount).slice(0,4).map(city=>{const cityImage=city.cover||published.find(place=>place.city===city.name)?.images[0]||cityFallbackImages[city.name]||places[0].images[0];return <Link to={`/city/${encodeURIComponent(city.id)}`} key={city.id} className="group relative min-h-28 overflow-hidden rounded-[20px] bg-stone-200 shadow-soft transition active:scale-95"><img src={cityImage} alt={`${city.name}城市风景`} className="absolute inset-0 h-full w-full object-cover transition duration-500 group-hover:scale-105"/><span aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-black/35 via-black/5 to-transparent"/><div className="absolute inset-x-3 bottom-3 z-10"><b className="text-sm text-white">{city.name}</b><p className="mt-1 text-[11px] text-white/80">{city.placeCount} 个地点 · {city.routeCount} 条路线</p></div></Link>})}</div>:<div className="rounded-[20px] bg-white px-5 py-8 text-center text-sm text-stone-400 shadow-soft">后台录入城市后会显示在这里</div>}</Section>
