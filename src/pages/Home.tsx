...
export function HomePage(){
 ...
 return <main className="min-h-[100dvh] bg-paper pb-28">
  <header className="px-5 pt-8"><h1 className="xj-display text-5xl font-black">循迹 <span className="text-2xl text-[#f4b321]">✦ ✦</span></h1><p className="mt-3 text-sm leading-6 text-stone-500">沿着喜欢的人，重演认识一座城市</p><div className="mt-5 grid grid-cols-[1fr_56px] gap-3"><SearchBox onClick={()=>nav('/search')}/><button className="xj-search xj-press flex items-center justify-center rounded-[22px]"><Bell size={21} strokeWidth={1.75}/></button></div></header>
  ...
  <Section title="同款地点" link="全部地点"><div className="grid grid-cols-2 gap-3">{featured.map(({place,relation,idol})=><article key={place.id} className="xj-surface xj-press xj-lift overflow-hidden"><Link to={`/place/${place.id}`}><img src={place.images[0]} className="h-28 w-full object-cover"/><div className="p-3"><h3 className="truncate font-bold">{place.name}</h3><p className="mt-1 truncate text-[11px] text-stone-500">{place.city||'深圳'} · {place.address.slice(0,5)}</p><p className="mt-2 text-xs">{idol} <span className="rounded-full bg-orange-50 px-2 py-1 text-[10px] text-orange-700">{relation.relationType==='filming'?'取景地':'公开分享'}</span></p><p className="mt-2 truncate text-[10px] text-stone-400">#城市漫游 #公开可查</p><div className="mt-2"><CredibilityBadge level={relation.credibility}/></div></div></Link><div className="grid grid-cols-2 border-t border-stone-100 text-[11px] font-semibold"><button onClick={()=>toggle(place.id)} className="flex min-h-11 items-center justify-center gap-1 border-r active:bg-stone-50"><Heart size={14} fill={favs.includes(place.id)?'currentColor':'none'}/>收藏</button><button onClick={()=>add(place.id)} className="flex min-h-11 items-center justify-center gap-1 active:bg-stone-50"><Plus size={14}/>路线</button></div></article>)}</div></Section>
