...
export function MapExplorePage(){
 ...
 return <main className="xj-page relative h-[100dvh] overflow-hidden bg-paper lg:p-6">
  <div ref={container} className="absolute inset-0 lg:inset-y-6 lg:left-6 lg:right-[42%] lg:overflow-hidden lg:rounded-[30px] lg:shadow-soft"/>
  ...
  <header className="absolute inset-x-4 top-4 z-20 flex flex-col gap-3 lg:left-10 lg:right-[45%] lg:top-10">
  ...
  <button type="button" onClick={()=>{haptic();locate()}} aria-label="定位到当前位置" className="xj-glass xj-press absolute right-5 top-[190px] z-20 flex h-12 w-12 items-center justify-center rounded-full border border-white/50 shadow-soft lg:right-[45%] lg:top-[190px]"><LocateFixed size={21}/></button>
  <section className={`xj-sheet absolute inset-x-0 bottom-[64px] z-30 rounded-t-[30px] ${sheetExpanded?'h-[min(62dvh,560px)]':'h-[286px]'} lg:inset-y-6 lg:left-auto lg:right-6 lg:bottom-auto lg:w-[calc(42%-1.5rem)] lg:rounded-[30px] lg:!h-[calc(100dvh-3rem)]`} style={{transform:`translateY(${sheetDrag>0?sheetDrag:sheetDrag*.18}px)`}}>
