...
function CityImageCell({city}:{city:ManagedCity}){
 const tones=['bg-orange-100 text-orange-700','bg-sky-100 text-sky-700','bg-violet-100 text-violet-700','bg-rose-100 text-rose-700']
 const tone=tones[[...city.name].reduce((sum,char)=>sum+char.charCodeAt(0),0)%tones.length]
 return city.cover?<img src={city.cover} alt={city.name} className="h-12 w-12 rounded-xl object-cover"/>:<div className={`flex h-12 w-12 items-center justify-center rounded-xl text-lg font-black ${tone}`}>{city.name.slice(0,1)}</div>
}
 ...
 return <AdminShell title={edit?'编辑爱豆':'录入爱豆'} subtitle="维护地点可关联的爱豆基础资料">
  <form onSubmit={submit} className="max-w-4xl rounded-2xl bg-white p-8 shadow-sm">
   <div className="grid grid-cols-2 gap-6">
    ...
    <Field label="头像（选填）"><input className={control} value={form.avatar} onChange={e=>setForm({...form,avatar:e.target.value})} placeholder="可粘贴图片 URL"/><ImageUploadControl value={form.avatar} onChange={avatar=>setForm({...form,avatar})} folder="idols"/></Field>
   </div>
 ...
 return <AdminShell title={edit?'编辑作品':'录入作品'} subtitle="维护电影、剧集、综艺、书籍与音乐资料">
  <form onSubmit={submit} className="max-w-4xl rounded-2xl bg-white p-8 shadow-sm">
   ...
   <div className="mt-6 grid grid-cols-2 gap-6">
    ...
    <div className="col-span-2"><Field label="作品封面（选填）"><input className={control} value={form.cover} onChange={event=>setForm({...form,cover:event.target.value})} placeholder="可粘贴图片 URL"/><ImageUploadControl value={form.cover} onChange={cover=>setForm({...form,cover})} folder="works"/></Field></div>
...
export function AdminWorkListPage(){
 ...
 useEffect(()=>recalculateBasicCounts(places),[places])
 const safeWorks=store.works.filter((work):work is ManagedWork=>Boolean(work)&&typeof work.name==='string')
 const count=(work:ManagedWork)=>places.filter(place=>place.status==='published'&&placeHasWork(place,work.id,work.name)).length
 return <AdminShell title="作品管理" subtitle="维护地点可关联的作品资料"><Toolbar add="/admin/work" label="录入作品"/><Table heads={['封面','名称','类型','年份','关联爱豆','关联地点数','操作']}>{safeWorks.map(x=><tr key={x.id} className="border-t border-slate-100"><td className="px-6 py-4"><ImageCell url={x.cover||''} name={x.name}/></td><td className="font-bold">{x.name}</td><td>{types[x.type]||'其他'}</td><td>{x.year||'未填'}</td><td>{(x.relatedIdolNames?.length?x.relatedIdolNames:(x.relatedIdolIds||[]).map(id=>store.idols.find(i=>i.id===id)?.name).filter(Boolean)).join('、')||'未关联'}</td><td><CountButton value={count(x)} onClick={()=>setSelected({id:x.id,name:x.name})}/></td><td className="pr-6"><Actions edit={`/admin/work?edit=${x.id}`} remove={()=>confirm(`确定删除“${x.name}”吗？`)&&store.removeWork(x.id)}/></td></tr>)}</Table>{selected&&<RelationPlacesModal kind="work" entity={selected} places={places} onClose={()=>setSelected(null)}/>}</AdminShell>
}
